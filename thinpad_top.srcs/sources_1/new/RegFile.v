...
//输入
    //读端口1 
        ...
        input wire [`RegsAddrBus]     rf_in_raddr1        ,// 1号端口读地址 
    //读端口2
        input wire                    rf_in_re2           , //2号端口读使能信号 
        input wire [`RegsAddrBus]     rf_in_raddr2        ,// 2号端口读地址 
    //写端口 
        input wire                    rf_in_we            ,//写使能信号
        input wire [`RegsAddrBus]     rf_in_waddr         , //写地址
        input wire [`RegsDataBus]     rf_in_wdata         ,//写数据
    
//输出
    output reg [`RegsDataBus]         rf_out_rdata1       , // 1号端口读数据 
    output reg [`RegsDataBus]         rf_out_rdata2         // 2号端口读数据 
    );
